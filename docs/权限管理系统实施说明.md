...
1. **数据库表创建**
   ...
   - 位置：`backend/scripts/create-permission-tables.sql`

2. **权限同步脚本**
   - 从路由配置同步到数据库
   - 位置：`backend/scripts/sync-routes-to-permissions.js`

3. **权限管理 API**
   - 获取权限列表
   - 获取用户/组权限
   - 分配/移除权限
   - 位置：`backend/routes/permission.js`
...
6. **403 页面**
   - 无权限访问提示页面
   - 位置：`src/views/Error/403.vue`（已存在）
...
```bash
# 在服务器上执行
sqlcmd -S localhost -d jiuhuanDB -i /opt/jh-craftsys/source/backend/scripts/create-permission-tables.sql
```
...
1. **AD 数据查询 API**
   ...
   - 查询"模具"OU 下的组列表
   - 查询用户所属组
   - 位置：`backend/routes/ad.js`（待创建）

2. **组权限查询完善**
   - 当前只查询用户直接权限
   - 需要添加组权限查询逻辑
   - 位置：`backend/routes/auth.js` 和 `backend/routes/permission.js`

3. **权限管理前端页面**
   - 权限分配界面
   - 用户/组选择器
   - 权限树形选择器
   - 位置：`src/views/Authorization/Permission/`（待创建）
...
- `backend/scripts/create-permission-tables.sql` - 数据库表创建脚本
- `backend/scripts/sync-routes-to-permissions.js` - 权限同步脚本
- `backend/routes/permission.js` - 权限管理 API
- `backend/routes/auth.js` - 登录接口（已修改，添加权限查询）
- `backend/server.js` - 服务器配置（已修改，添加权限路由）

### 前端文件

- `src/permission.ts` - 路由守卫（已修改，添加权限验证）
- `src/router/index.ts` - 路由配置（已修改，添加 403 路由）
- `src/views/Error/403.vue` - 403 页面（已存在）
