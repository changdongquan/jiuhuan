...
async function analyzeExcel(filePath) {
  try {
    ...
    data.forEach((row, index) => {
      ...
      const validFields = [
        '项目编号', '项目名称', '客户模号', '产品材质', '模具穴数', '项目状态',
        '设计师', '中标日期', '产品3D确认', '图纸下发时间', '计划首样日期',
        '首次送样日期', '移模日期', '制件厂家', '进度影响原因', '前模材质',
        '后模材质', '模具尺寸', '产品尺寸', '产品重量', '收缩率', '产品颜色',
        '模具重量', '流道类型', '机台吨位', '备注', '滑块材质', '锁模力',
        '定位圈', '容模量', '拉杆间距', '成型周期', '送样时间', '浇口类型',
        '浇口数量', '料柄重量', '流道数量', '费用出处'
      ]
      ...
      try {
        ...
        if (checkResult[0].count > 0) {
          ...
          Object.keys(item).forEach(key => {
            if (key !== '项目编号' && key !== 'SSMA_TimeStamp' && item[key] !== undefined && item[key] !== null && item[key] !== '') {
          ...
          Object.keys(item).forEach(key => {
            if (key !== 'SSMA_TimeStamp' && item[key] !== undefined && item[key] !== null && item[key] !== '') {
              fields.push(`[${key}]`)
              values.push(`@${key}`)
              params[key] = item[key]
            }
