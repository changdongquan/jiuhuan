...
将 `backend/jiuhuan-backend.service` 文件上传到服务器的 `/opt/jh-craftsys/source/backend/` 目录。
...
```bash
sudo cp /opt/jh-craftsys/source/backend/jiuhuan-backend.service /etc/systemd/system/
```
