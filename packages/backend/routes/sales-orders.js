...
const ensureSalesOrderCustomerSoftDeleteColumns = async (poolOrTx) => {
...
router.get('/customer-options', async (req, res) => {
  try {
    ...
    await ensureSalesOrderCustomerSoftDeleteColumns(pool)
    ...
    console.error('获取销售订单客户选项失败:', error)
    return res.status(500).json({
      code: 500,
      success: false,
      message: '获取销售订单客户选项失败: ' + (error?.message || '未知错误')
    })
  }
})
