...
router.get('/invoices/list', async (req, res) => {
  try {
    ...
    if (itemCode) {
      where.push('(i.单据编号 LIKE @itemCode OR i.发票号码 LIKE @itemCode OR d.项目编号 LIKE @itemCode)')
...
router.get('/receipts/list', async (req, res) => {
  try {
    ...
    if (itemCode) {
      where.push('(r.单据编号 LIKE @itemCode OR r.项目编号 LIKE @itemCode OR CONVERT(NVARCHAR(50), r.明细ID) LIKE @itemCode)')
      params.itemCode = `%${String(itemCode).trim()}%`
    }
