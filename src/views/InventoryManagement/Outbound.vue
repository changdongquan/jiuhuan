  ...
  <div class="p-4 space-y-4">
    ...
    <el-card shadow="never" class="search-card">
      ...
      >
        ...
        <el-form-item :label="t('inventoryOutbound.customer')">
          <el-input
        ...
        <el-form-item :label="t('inventoryOutbound.warehouse')">
          <el-select
        ...
        <el-form-item :label="t('inventoryOutbound.status')">
          <el-select
        ...
        <el-form-item :label="t('inventoryOutbound.outboundDate')">
          <el-date-picker
            v-model="queryForm.outboundDateRange"
            type="daterange"
        ...
        <el-form-item>
          ...
          <el-button type="success" @click="handleAdd">
            ...
            {{ t('inventoryOutbound.addOutbound') }}
          </el-button>
        ...
        <el-card shadow="hover" class="summary-card total">
          <div class="summary-content">
            ...
            <div class="summary-info">
              <div class="summary-title">{{ t('inventoryOutbound.totalOutbound') }}</div>
              <div class="summary-value">{{ summary.totalOutbound }}</div>
            </div>
          </div>
        </el-card>
      </el-col>
      <el-col :xs="24" :sm="12" :lg="6">
        <el-card shadow="hover" class="summary-card amount">
          <div class="summary-content">
            ...
            <div class="summary-info">
              <div class="summary-title">{{ t('inventoryOutbound.totalValue') }}</div>
      ...
      <el-col :xs="24" :sm="12" :lg="6">
        <el-card shadow="hover" class="summary-card pending">
          <div class="summary-content">
            ...
            <div class="summary-info">
              <div class="summary-title">{{ t('inventoryOutbound.pendingCount') }}</div>
      ...
      <el-col :xs="24" :sm="12" :lg="6">
        <el-card shadow="hover" class="summary-card completed">
          <div class="summary-content">
            ...
            <div class="summary-info">
              <div class="summary-title">{{ t('inventoryOutbound.completedCount') }}</div>
    ...
    >
      ...
      >
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item :label="t('inventoryOutbound.outboundDate')" prop="outboundDate">
              <el-date-picker
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item :label="t('inventoryOutbound.status')" prop="status">
          ...
          <div class="material-details">
            <div class="material-header">
              <el-button type="primary" size="small" @click="handleAddMaterial">
                <el-icon><Plus /></el-icon>
                {{ t('inventoryOutbound.addMaterial') }}
              </el-button>
            </div>
        ...
        <el-form-item :label="t('inventoryOutbound.remark')" prop="remark">
          <el-input
            v-model="formData.remark"
            type="textarea"
            :rows="3"
            :placeholder="t('inventoryOutbound.remarkPlaceholder')"
          />
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <el-dialog
      ...
      :title="t('inventoryOutbound.outboundDetail')"
      width="800px"
      :close-on-click-modal="false"
    >
      <el-descriptions :column="2" border>
        <el-descriptions-item :label="t('inventoryOutbound.documentNo')">
          {{ currentOutbound?.documentNo }}
        </el-descriptions-item>
        <el-descriptions-item :label="t('inventoryOutbound.customer')">
          {{ currentOutbound?.customer }}
        </el-descriptions-item>
        <el-descriptions-item :label="t('inventoryOutbound.warehouse')">
          {{ currentOutbound?.warehouse }}
        </el-descriptions-item>
        <el-descriptions-item :label="t('inventoryOutbound.outboundDate')">
          {{ currentOutbound?.outboundDate }}
        </el-descriptions-item>
        <el-descriptions-item :label="t('inventoryOutbound.operator')">
          {{ currentOutbound?.operator }}
        </el-descriptions-item>
        <el-descriptions-item :label="t('inventoryOutbound.status')">
          <el-tag :type="statusTagMap[currentOutbound?.status || 'pending'].type">
            {{ statusTagMap[currentOutbound?.status || 'pending'].label }}
          </el-tag>
        </el-descriptions-item>
        <el-descriptions-item :label="t('inventoryOutbound.materialCount')">
          {{ currentOutbound?.materialCount }}
        </el-descriptions-item>
        <el-descriptions-item :label="t('inventoryOutbound.totalValue')">
          ¥ {{ formatAmount(currentOutbound?.totalValue || 0) }}
        </el-descriptions-item>
        <el-descriptions-item :label="t('inventoryOutbound.remark')" :span="2">
          {{ currentOutbound?.remark || '-' }}
        </el-descriptions-item>
      </el-descriptions>
    </el-dialog>
  </div>
...
type OutboundStatus = 'pending' | 'confirmed' | 'completed' | 'cancelled'
...
const currentOutbound = ref<OutboundRecord | null>(null)
...
const queryForm = reactive({
  ...
  outboundDateRange: [] as [string, string] | []
})
...
// 表单数据
const formData = reactive<Partial<OutboundRecord>>({
  documentNo: '',
  customer: '',
  warehouse: '',
  materialCount: 0,
...
// 状态选项
const statusOptions = [
  { label: t('inventoryOutbound.statusPending'), value: 'pending' },
  { label: t('inventoryOutbound.statusConfirmed'), value: 'confirmed' },
  { label: t('inventoryOutbound.statusCompleted'), value: 'completed' },
  { label: t('inventoryOutbound.statusCancelled'), value: 'cancelled' }
]
...
> = {
  pending: { label: t('inventoryOutbound.statusPending'), type: 'info' },
  confirmed: { label: t('inventoryOutbound.statusConfirmed'), type: 'warning' },
  completed: { label: t('inventoryOutbound.statusCompleted'), type: 'success' },
  cancelled: { label: t('inventoryOutbound.statusCancelled'), type: 'danger' }
}
...
const dialogTitle = computed(() => {
  return formData.id ? t('inventoryOutbound.editOutbound') : t('inventoryOutbound.addOutbound')
})
...
const filteredOutbounds = computed(() => {
  let result = outbounds.value.filter((outbound) => {
    const documentMatch = queryForm.documentNo
      ...
      : true
    const customerMatch = queryForm.customer
      ? outbound.customer.toLowerCase().includes(queryForm.customer.toLowerCase())
      : true
    const warehouseMatch = queryForm.warehouse ? outbound.warehouse === queryForm.warehouse : true
  ...
  if (sortConfig.prop && sortConfig.order) {
    result.sort((a, b) => {
      const aVal = a[sortConfig.prop as keyof OutboundRecord]
      const bVal = b[sortConfig.prop as keyof OutboundRecord]
...
const summary = computed(() => {
  const result = {
    totalOutbound: outbounds.value.length,
    totalValue: 0,
    pendingCount: 0,
    completedCount: 0
  }
  outbounds.value.forEach((outbound) => {
    result.totalValue += outbound.totalValue
    if (outbound.status === 'pending') result.pendingCount += 1
    if (outbound.status === 'completed') result.completedCount += 1
  })
  return result
})
...
const handleRowDoubleClick = (row: OutboundRecord) => {
  handleEdit(row)
}
...
const handleDelete = async (row: OutboundRecord) => {
  try {
    await ElMessageBox.confirm(
      t('inventoryOutbound.deleteConfirm', { documentNo: row.documentNo }),
      t('common.warning'),
      {
...
const handleBatchConfirm = () => {
  ElMessage.info(t('inventoryOutbound.batchConfirmSuccess'))
}
